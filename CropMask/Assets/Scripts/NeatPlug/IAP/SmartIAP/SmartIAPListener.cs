...
public class SmartIAPListener : MonoBehaviour {
	...
	void OnOwnedItemReported(string sku) {
       ...
       if(gameController!=null)
        {
            gameController.PaidUserDetected(sku);
        }
	...
	void OnItemAlreadyOwned(string sku) {
        ...
        {
            gameController.PaidUserDetected(sku);
            gameController.InstantiateInfoPopup("You are already a paid user");
            gameController.selectDressController.CheckForChanges();
        }
